...
echo'
<label posn="47 16 11" halign="center" text="'.strtoupper(trim($_SESSION["statistik"][$la])).'" />
...
for($i=0;$i<count($array);$i++) {
	if ($_SESSION["sta"]!=$i) {
		...
		<label style="TextCardInfoSmall" manialink="'.navi("page").'sta='.$i.'" halign="center" posn="45.9 '.($p-0.75).' 11" sizen="30.7 3" text="'.$array[$i].'" />
		';
		$p-=3;
		$s-=3;
	} else {
		...
		case "0" :
			$text='
			<label posn="'.(45.9-14).' '.($p-3.3).' 11" text="'.trim($_SESSION["klicks"][$la]).' : '.$zahll4.'" />
			<label halign="right" posn="'.(45.9+14).' '.($p-3.3).' 11" text="'.trim($_SESSION["counter"][$la]).' : '.$zahl4.'" />
			<label posn="'.(45.9-14).' '.($p-5.7).' 11" text="'.trim($_SESSION["besucher online"][$la]).' : '.$online.'" />
			<label posn="'.(45.9-14.5).' '.($p-8.3).' 11" sizen="14.4 2" text="'.$besucher1[0].'" />
			<label posn="'.(45.9+14.5).' '.($p-8.3).' 11" sizen="14.4 2" halign="right" text="'.$besucher1[1].'" />
			<label posn="'.(45.9-14.5).' '.($p-10.7).' 11" sizen="14.4 2" text="'.$besucher1[2].'" />
			<label posn="'.(45.9+14.5).' '.($p-10.7).' 11" sizen="14.4 2" halign="right" text="'.$besucher1[3].'" />
			<label posn="'.(45.9-14.5).' '.($p-11.1).' 11" sizen="14.4 2" text="'.$besucher1[4].'" />
			<label posn="'.(45.9+14.5).' '.($p-11.1).' 11" sizen="14.4 2" halign="right" text="'.$besucher1[5].'" />
			';
			break;
		case "1" :
			$ii=3.2;
			...
			$ii2=1.05;
			$text='
			<label posn="'.(45.9-14).' '.($p-$ii-$ii2).' 11" sizen="18 4.6" valign="center" >'.$best_0[$r].'</label>
			<label posn="'.(45.9+14).' '.($p-$ii-$ii2).' 11" sizen="36 4.6" halign="right" valign="center" >'.$best_1[$r].'         </label>
		...
		case "2" :
			$mm=2.3;
			$text='
		<label posn="'.(45.9-14).' '.($p-2.1).' 11" sizen="22 4.6" >'.trim($_SESSION["letzter spender"][$la]).' :</label>
		<label posn="'.(45.9-14).' '.($p-7).' 11" sizen="16 4.6" halign="left" valign="center" >'.$bad.'$z</label>
		<label posn="'.(45.9+11.5).' '.($p-7).' 11" sizen="16 4.6" halign="right" valign="center" >'.$badn[1].'</label>
		<quad posn="'.(45.9+11.5).' '.($p-7).' 4" sizen="3 3" valign="center" style="Icons128x128_1" substyle="Coppers"/>

		<label posn="'.(45.9-14).' '.($p-8.7-$mm).' 11" sizen="22 4.6" valign="center" >'.trim($_SESSION["summe spenden"][$la]).' :</label>
		<label posn="'.(45.9+11.5).' '.($p-8.7-$mm).' 11" sizen="16 4.6" halign="right" valign="center" >'.$all.'$z</label>
		<quad posn="'.(45.9+11.5).' '.($p-8.7-$mm).' 3" sizen="3 3" valign="center" style="Icons128x128_1" substyle="Coppers"/>

		<label posn="'.(45.9-14).' '.($p-11.4-$mm).' 11" sizen="22 4.6" valign="center" >'.trim($_SESSION["anzahl spenden"][$la]).' :</label>
		<label posn="'.(45.9+11.5).' '.($p-11.4-$mm).' 11" sizen="16 4.6" halign="right" valign="center" >'.$anzahl.'$z</label>
		<quad posn="'.(45.9+11.5).' '.($p-11.4-$mm).' 3" sizen="3.4 3.4" valign="center" style="Icons128x128_1" substyle="MedalCount"/>

		<label posn="'.(45.9-14).' '.($p-14.1-$mm).' 11" sizen="22 4.6" valign="center" >'.trim($_SESSION["anzahl spender"][$la]).' :</label>
		<label posn="'.(45.9+11.5).' '.($p-14.1-$mm).' 11" sizen="16 4.6" halign="right" valign="center" >'.$bzahl4.'$z</label>
		...
		case "3" :
			...
			for($a=0;$a<4;$a++) {
				$text.='
				<label textcolor="FFF" posn="'.(45.9-14).' '.($p-3-3*$a).' 11" sizen="22 4.6" >'.$m_name[$a].'</label>
				<label textcolor="FFF" posn="'.(45.9+13).' '.($p-3-3*$a).' 11" sizen="16 4.6" halign="right" >'.$m_zahl[$a].'$z</label>	
				';
			}
			$text.='
			<label posn="'.(45.9-14).' '.($p-3-12).' 11" sizen="11 4.6" >'.trim($_SESSION["name der musik"][$la]).'</label>
			<label posn="'.(45.9+14).' '.($p-3-12).' 11" sizen="13 4.6" halign="right" >'.ltrim($_SESSION["Wie oft gew&#228;hlt"][$la]).'</label>	
			';
			break;
		}
		echo '
		<quad halign="center" posn="45.9 '.$p.' 1.800003" sizen="30.7 '.(3+$s).'" style="Bgs1InRace" substyle="BgWindow1" />
		<label style="TextCardInfoSmall" halign="center" posn="45.9 '.($p-0.3).' 11" sizen="30.7 3" text="'.$array[$i].'" />
		'.$text.'
		';
		$p-=18.1;
		$s-=18.1;
